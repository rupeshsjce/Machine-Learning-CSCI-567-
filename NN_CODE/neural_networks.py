...
class linear_layer:
    ...
    def __init__(self, input_D, output_D):
        ...
        self.params['W'] = np.random.normal(
            mu, sigma, size=(input_D, output_D))  # (784,1000)
        self.params['b'] = np.random.normal(
            mu, sigma, size=(1, output_D))  # (1,1000)
        ...
        #   - self.gradient['b']
        ###############################################################################################
        self.gradient['W'] = np.zeros((input_D, output_D))  # (784, 1000)
        self.gradient['b'] = np.zeros((1, output_D))  # (1, 1000)
    ...
    def backward(self, X, grad):
        ...
        #################################################################################################
        # do we need to take mean of the gradient
...
class relu:
    ...
    def backward(self, X, grad):
        ...
        backward_output = grad * \
            ((self.mask > 0).astype(float)
             )  # maybe self.params['W'] is needed.
...
class tanh:
    ...
    def backward(self, X, grad):
        ...
        ####################################################################################################
        backward_output = grad * \
            ((1 - np.tanh(X) ** 2))  # maybe self.params['W'] is needed.
...
def miniBatchGradientDescent(model, momentum, _alpha, _learning_rate):
    ...
    for module_name, module in model.items():
        #print("module_name, module : ", module_name, module)
...
def main(main_params):
    ...
    model['L1'] = linear_layer(input_D=d, output_D=num_L1)  # (784,1000)
