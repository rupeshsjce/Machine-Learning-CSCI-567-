...
class KNN:
    ...
    def get_k_neighbors(self, point):
        ...
        for feature in self.x_train:
            distance_x_point.append(self.distance_function(feature,point))
        
        
        iterate = len(self.x_train) if self.k>len(self.x_train) else self.k 
        kNearestLabels = np.argsort(np.array(distance_x_point), kind = 'stable')
        return kNearestLabels[:iterate]
    ...
    def predict(self, features):
        ...
        :return: List[int]
        """
        idx = [self.get_k_neighbors(feature) for feature in features]
        tmp_lab = [[self.y_train[j] for j in i] for i in idx]
        plabel = [Counter(w) for w in tmp_lab]
        predict = [max(s,key = s.get) for s in plabel]
